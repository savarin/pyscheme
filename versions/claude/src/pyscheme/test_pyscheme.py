...
from pyscheme.pyscheme import (
    evaluate, Environment, create_global_environment,
    SchemeError, UndefinedSymbolError, InvalidExpressionError, ArityError,
    make_memoized, validate_expression
)
...
class TestEdgeCases:
    ...
    def test_function_as_value(self):
        ...
        apply_twice = evaluate(
            ("lambda", ("f", "x"), ("f", ("f", "x"))), env
        )
        inc = evaluate(("lambda", ("x",), ("+", "x", 1)), env)
        assert apply_twice(inc, 5) == 7  # (5 + 1) + 1


# Import the original test file to ensure backward compatibility
from pyscheme.test_pyscheme import *
